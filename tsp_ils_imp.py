...
# --------------------------------------------------------------------
#   improved Iterated Local Search for TSP
#
...
def iterated_local_search(tsp, work, time_limit):
    ...
    # initialize temperature
    min_temp, max_temp = init_temp(work)

    # initialize seed working data
    seed_work = Work(tsp)
    seed_work.copy(work)
    ...
    while cur_time - start_time < time_limit:
        ...
        if cur_work.obj < work.obj:
            work.copy(cur_work)
            print('{}\t{}*\t{} ({})\t{:.2f}'.format(cnt,cur_work.obj,work.obj,seed_work.obj,cur_time-start_time))
        ...
        # update seed working data
        delta = cur_work.obj - seed_work.obj
        if delta < 0 or math.exp(-delta/temp) > random.random():
            seed_work.copy(cur_work)
