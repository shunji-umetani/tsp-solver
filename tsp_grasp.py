...
#   work(I/O): working data
# --------------------------------------------------------------------
def random_nearest_neighbor(tsp, work):
    # nearest neighbor
    work.tour = [i for i in range(tsp.num_node)]
    for i in range(1,tsp.num_node):
        # initialize heap
        top_k = []
        # find top-k nearest unvisited node
        for j in range(i,tsp.num_node):
            dist = tsp.dist((work.tour)[i-1],(work.tour)[j])
            if len(top_k) < TOP_K_SIZE:
                heapq.heappush(top_k, (-dist, j))
            elif -dist > top_k[0][0]:
                heapq.heapreplace(top_k, (-dist, j))
        # set randomly selected unvisited node from top-k
        idx = top_k[random.randrange(min(TOP_K_SIZE,len(top_k)))][1]
        (work.tour)[i], (work.tour)[idx] = (work.tour)[idx], (work.tour)[i]
        # clear heap
        top_k.clear()
    # initialize position of nodes in tour
    work.set_pos()
...
def grasp(tsp, work, time_limit):
    ...
    while cur_time - start_time < time_limit:
        ...
        random_nearest_neighbor(tsp, cur_work)
        ...
        if cur_work.obj < work.obj:
            work.tour = (cur_work.tour)[:]
            work.obj = cur_work.obj
...
def main(argv=sys.argv):
    ...
    # solve TSP
    work = Work(tsp)
    nearest_neighbor(tsp, work)  # nearest neighbor
    grasp(tsp, work, args.time)  # GRASP
    work.write(tsp)
